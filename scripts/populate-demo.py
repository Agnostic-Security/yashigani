...
def _totp(secret: str) -> str:
    ...
    Delegates to the self-contained pyotp-backed _role_totp (28406b0a) —
    matches auth/totp.py ROLE_TOTP_ALGO/ROLE_TOTP_DIGITS without importing
    yashigani.auth.totp (that import pulls the fastapi chain, absent from
    the standalone demo venv)."""
...
def _user_totp_sha256(secret: str) -> str:
    """User-tier TOTP (SHA256/6-digit). Delegates onto the self-contained
    pyotp-backed _role_totp (28406b0a) — collapses the previous hand-rolled
    HMAC/base32 implementation onto the single source of truth also used for
    admin TOTP (ROLE_TOTP_ALGO["user"] / ROLE_TOTP_DIGITS["user"] == SHA256/6,
    matching src/yashigani/auth/totp.py exactly, without importing it)."""
...
def step14_verify_aspen() -> None:
    ...
    code = _totp(ASPEN_TOTP_SECRET)  # backup admin — also SHA-512/8-digit (admin-tier _role_totp)
