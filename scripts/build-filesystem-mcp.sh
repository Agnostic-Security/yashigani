#!/usr/bin/env bash
# scripts/build-filesystem-mcp.sh — build the filesystem-mcp bundle image
#                                   and emit the digest for bundles/filesystem-mcp.yaml.
#
# Usage:
#   bash scripts/build-filesystem-mcp.sh
#   bash scripts/build-filesystem-mcp.sh --platform linux/amd64
#   bash scripts/build-filesystem-mcp.sh --platform linux/amd64,linux/arm64  # multi-arch via buildx
#
# Outputs:
#   - Built image tagged as:
#       registry.yashigani.internal/bundles/mcp-filesystem:latest
#   - On success, prints the sha256 digest to stdout for copy-paste into
#     bundles/filesystem-mcp.yaml spec.image.digest.
#
...
#
# last-updated: 2026-05-29T00:00:00+01:00 (feat(p3): build-filesystem-mcp.sh — T2 topology build helper)
...
# ── Build ─────────────────────────────────────────────────────────────────────
printf '[build-filesystem-mcp] Building %s:%s\n' "$IMAGE_NAME" "$IMAGE_TAG"
printf '[build-filesystem-mcp] Runtime: %s\n' "$CONTAINER_CMD"
printf '[build-filesystem-mcp] Dockerfile: %s\n' "$DOCKERFILE"
printf '[build-filesystem-mcp] Context: %s\n' "$REPO_ROOT"
...
if [[ -n "$PLATFORM" ]]; then
  ...
  if [[ "$PLATFORM" == *","* ]]; then
    printf '[build-filesystem-mcp] Multi-arch build via buildx: %s\n' "$PLATFORM"
    ...
    "$CONTAINER_CMD" buildx build \
      ...
      --push \
      "$REPO_ROOT"
    printf '\n[build-filesystem-mcp] Multi-arch push complete.\n'
    printf '[build-filesystem-mcp] Inspect the registry digest with:\n'
...
# ── Capture digest ────────────────────────────────────────────────────────────
printf '\n[build-filesystem-mcp] Capturing image digest...\n'
...
if [[ -z "$DIGEST" ]]; then
  ...
  # The operator must push the image to get a stable digest for bundles/filesystem-mcp.yaml.
  IMAGE_ID="$("$CONTAINER_CMD" inspect --format '{{.Id}}' "${IMAGE_NAME}:${IMAGE_TAG}" 2>/dev/null || true)"
  printf '\n[build-filesystem-mcp] WARNING: No RepoDigest available (image not pushed to registry).\n'
  printf '[build-filesystem-mcp] Image ID: %s\n' "$IMAGE_ID"
  printf '\n[build-filesystem-mcp] To get a stable digest for bundles/filesystem-mcp.yaml:\n'
  printf '  1. Push the image: docker push %s:%s\n' "$IMAGE_NAME" "$IMAGE_TAG"
  printf '  2. Get the digest: docker manifest inspect %s:%s | jq -r '"'"'.[0].Digest'"'"'\n' "$IMAGE_NAME" "$IMAGE_TAG"
  printf '  3. Update bundles/filesystem-mcp.yaml spec.image.digest with the sha256 value.\n'
else
  printf '\n[build-filesystem-mcp] Image digest (copy into bundles/filesystem-mcp.yaml):\n'
  printf '  spec.image.digest: "%s"\n\n' "$DIGEST"
  printf '[build-filesystem-mcp] Full image reference:\n'
  printf '  %s:%s@%s\n\n' "$IMAGE_NAME" "$IMAGE_TAG" "$DIGEST"
fi

printf '[build-filesystem-mcp] Build complete.\n'
printf '[build-filesystem-mcp] M6 reminder: update bundles/filesystem-mcp.yaml spec.image.digest\n'
