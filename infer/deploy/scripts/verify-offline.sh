...
#   1. hadolint            — every Dockerfile.infer-*
#   2. shellcheck           — every *.sh under infer/deploy/
#   3. helm lint            — infer/deploy/helm/yashigani-infer
#   4. helm template        — render with default + a GPU-backend values override
#   5. caddy adapt           — Caddyfile.infer-front (assert :443-style discipline: every
#                              dial explicit, no :80, no insecure_skip_verify)
#   6. jq seccomp validity   — infer/deploy/scripts/verify-seccomp-json.sh
#   7. helm-mirror parity    — infer/deploy/scripts/sync-infer-deploy-artifacts-to-helm.sh --check
#
...
if command -v hadolint >/dev/null 2>&1; then
    rc=0
    for f in "$REPO_ROOT"/docker/Dockerfile.infer-*; do
        echo "--- $f ---"
...
echo
echo "== 3. helm lint =="
helm lint "$REPO_ROOT/helm/yashigani-infer"
record "helm lint (default values)" "$?"

echo
echo "== 4. helm template (default + cuda backend) =="
helm template infer-test "$REPO_ROOT/helm/yashigani-infer" >/dev/null
record "helm template (default: cpu backend)" "$?"
helm template infer-test "$REPO_ROOT/helm/yashigani-infer" \
    --set backend=cuda \
...
if command -v caddy >/dev/null 2>&1; then
    tmp_caddyfile="$(mktemp -d)/Caddyfile.infer-front.standalone"
    # Caddyfile.infer-front is a snippet+site-block designed to be `import`ed — wrap it in a
    ...
    cp "$REPO_ROOT/docker/Caddyfile.infer-front" "$tmp_caddyfile"
    adapt_out="$(caddy adapt --config "$tmp_caddyfile" --adapter caddyfile 2>&1)"
    adapt_rc=$?
    echo "$adapt_out" | head -5
    rc=0
    if [ "$adapt_rc" -ne 0 ]; then
        echo "caddy adapt FAILED to parse Caddyfile.infer-front:" >&2
        echo "$adapt_out" >&2
        rc=1
    else
        if grep -qE '":80"|:80"' "$REPO_ROOT/docker/Caddyfile.infer-front"; then
            echo "FAIL: :80 found in Caddyfile.infer-front" >&2
            rc=1
        fi
        if grep -qi "insecure_skip_verify" "$REPO_ROOT/docker/Caddyfile.infer-front"; then
            echo "FAIL: insecure_skip_verify found in Caddyfile.infer-front" >&2
            rc=1
        fi
        if ! grep -q ":11436" "$REPO_ROOT/docker/Caddyfile.infer-front"; then
...
"$REPO_ROOT/scripts/sync-infer-deploy-artifacts-to-helm.sh" --check
