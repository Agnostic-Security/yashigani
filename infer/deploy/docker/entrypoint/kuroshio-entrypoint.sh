...
#      /proc/self/oom_score_adj so the kernel OOM-killer takes a kuroshio container before
...
OOM_SCORE_ADJ="${YSG_KUROSHIO_OOM_SCORE_ADJ:-500}"
...
# "Deferred / coordination gaps"): infer/src/kuroshio/ has no ASGI entrypoint
...
# selection per YSG_KUROSHIO_ROLE) is Python control-plane code and belongs in Tom's
# package (e.g. `kuroshio.entrypoint:create_asgi_app`), not authored here.
#
# The env-var contract this deploy layer expects that module to honour:
#   YSG_KUROSHIO_ROLE                  classifier | chat | puller
#   YSG_KUROSHIO_BLOB_STORE_ROOT       (already exists — config.py)
#   YSG_KUROSHIO_LLAMA_SERVER_BINARY   default "llama-server"; ABSENT in the puller image
#   YSG_KUROSHIO_MAX_CTX               hard n_ctx ceiling (clamp, never reject)
#   YSG_KUROSHIO_MAX_CONCURRENCY       ResourceLimits.max_concurrent_requests
#   YSG_KUROSHIO_MAX_TOKENS_PER_REQUEST ResourceLimits.max_tokens_per_request
#   YSG_KUROSHIO_IDLE_UNLOAD_SECONDS   Supervisor idle-unload timeout
#   YSG_KUROSHIO_MAX_RESIDENT_MODELS   Supervisor LRU ceiling
#   YSG_KUROSHIO_KEEP_ALIVE_PIN        "true" for the classifier role (WARMUP-001 analog)
#   YSG_KUROSHIO_EXPECT_GPU            "true" on GPU-tagged deployments (healthz hard-fail gate)
#   YSG_KUROSHIO_N_GPU_LAYERS / YSG_KUROSHIO_OVERRIDE_TENSOR   MoE offload passthrough
#
...
if ! python3 -c "import kuroshio.entrypoint" 2>/dev/null; then
    echo "FATAL: kuroshio.entrypoint module not found — no ASGI wiring entrypoint" >&2
    echo "       exists yet in the kuroshio package. This is a documented build-" >&2
...
exec uvicorn kuroshio.entrypoint:create_asgi_app --factory \
