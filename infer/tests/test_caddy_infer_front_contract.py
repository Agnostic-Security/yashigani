...
"""Contract tests for the infer-front `/api/pull` URL-convention fix (Iris F1
cutover-prep map, Seam 5, her recommended option (b)) + the single-source
Helm mirror discipline for `Caddyfile.infer-front`.

Background: `backoffice/routes/models.py` builds pull URLs as
`base + "/api/pull"` (the ollama convention). If `base` ever points at the
engine's Caddy front, that resolves to `/infer/api/pull` — which the
pre-existing literal `handle_path /infer/pull*` route does NOT match (no
`/api` segment). Without a dedicated route it falls through the generic
`handle_path /infer/*` catch-all to infer-chat, whose `/api/pull` returns 501
(no `pull_resolver` on that role) — a silent pull failure at cutover. The fix
adds `handle_path /infer/api/pull*`, rewriting to `/api/pull` and reaching
the SAME puller upstream as the pre-existing `/infer/pull*` route.

Pins three invariants:
  (a) `/infer/api/pull*` is present, ordered ahead of the generic `/infer/*`
      catch-all, rewrites to `/api/pull`, and reaches the same infer-puller
      upstream as the literal `/infer/pull*` route.
  (b) `caddy adapt` succeeds against the canonical Caddyfile.
  (c) The canonical file and its Helm ConfigMap mirror
      (`helm/yashigani-infer/files/Caddyfile.infer-front`) stay byte-identical
      (`scripts/sync-infer-deploy-artifacts-to-helm.sh` single-source
      discipline).
...
_CANONICAL = _INFER_ROOT / "deploy" / "docker" / "Caddyfile.infer-front"
_MIRROR = _INFER_ROOT / "deploy" / "helm" / "yashigani-infer" / "files" / "Caddyfile.infer-front"
_SYNC_SCRIPT = _INFER_ROOT / "deploy" / "scripts" / "sync-infer-deploy-artifacts-to-helm.sh"
_CONFIGMAP_TEMPLATE = (
    _INFER_ROOT / "deploy" / "helm" / "yashigani-infer" / "templates" / "configmap-caddy-infer-front.yaml"
)
...
# ---------------------------------------------------------------------------
# (a) /infer/api/pull* route present, correctly ordered
# ---------------------------------------------------------------------------


def test_infer_api_pull_route_present_and_ordered_before_catchall() -> None:
    text = _CANONICAL.read_text(encoding="utf-8")
    assert "handle_path /infer/api/pull*" in text, (
        "\nCaddyfile.infer-front is missing the /infer/api/pull* route (Iris F1 "
        'Seam 5 fix, option (b)) — a caller using the ollama base+"/api/pull" '
        "URL convention will fall through the generic /infer/* catch-all to "
        "infer-chat, which 501s (no pull_resolver on that role)."
    )
    # Caddyfile handle/handle_path blocks are mutually exclusive, first-match:
    # the specific /infer/api/pull* route MUST be declared before the generic
    # /infer/* catch-all or it would never get a chance to match.
    assert text.index("handle_path /infer/api/pull*") < text.index("handle_path /infer/*"), (
        "/infer/api/pull* must be declared before the generic /infer/* catch-all in file order."
    )
...
def test_infer_api_pull_rewrites_to_api_pull_and_reaches_puller_upstream() -> None:
    ...
    and dials the SAME infer-puller upstream as the pre-existing literal
    /infer/pull* route (not infer-chat/infer-classifier)."""
    if _caddy_binary() is None:
        pytest.skip("caddy binary not found on PATH")
    config = _adapt_canonical()
    routes = config["apps"]["http"]["servers"]["srv0"]["routes"]

    api_pull_route = next((r for r in routes if "/infer/api/pull*" in _matcher_paths(r)), None)
    assert api_pull_route is not None, "no adapted route matches /infer/api/pull*"

    literal_pull_route = next((r for r in routes if "/infer/pull*" in _matcher_paths(r)), None)
    assert literal_pull_route is not None, "no adapted route matches /infer/pull* (pre-existing route missing?)"

    assert "/api/pull" in _rewrite_targets(api_pull_route), (
        f"/infer/api/pull* does not rewrite to /api/pull: {_rewrite_targets(api_pull_route)}"
    )

    api_pull_upstreams = _puller_upstreams(api_pull_route)
    literal_pull_upstreams = _puller_upstreams(literal_pull_route)
    assert api_pull_upstreams, "/infer/api/pull* has no reverse_proxy upstream at all"
    assert api_pull_upstreams == literal_pull_upstreams, (
        "/infer/api/pull* must reach the SAME puller upstream as the literal "
        f"/infer/pull* route: {api_pull_upstreams} != {literal_pull_upstreams}"
    )
    assert all("infer-puller" in dial for dial in api_pull_upstreams), (
        f"/infer/api/pull* upstream is not infer-puller (would silently route "
        f"to the wrong role — infer-chat/-classifier 501 on /api/pull): {api_pull_upstreams}"
    )
...
def test_helm_mirror_byte_identical_to_canonical() -> None:
    ...
    assert canonical == mirror, (
        "\nCaddyfile.infer-front DRIFT — the helm mirror differs from the "
...
def test_mutation_planted_drift_is_caught(tmp_path: Path) -> None:
    ...
    canonical = _read_bytes(_CANONICAL)
    planted = canonical + b"\n# planted drift\n"
    mutated_mirror = tmp_path / "Caddyfile.infer-front"
...
def test_sync_script_check_mode_catches_planted_drift(tmp_path: Path) -> None:
    ...
    mirror_dir = staged_deploy / "helm" / "yashigani-infer" / "files"
    ...
    assert result.returncode != 0, (
        "MUTATION TEST FAILED: sync-infer-deploy-artifacts-to-helm.sh --check "
...
def test_configmap_template_uses_files_get_not_reinlined_copy() -> None:
    ...
    assert '.Files.Get "files/Caddyfile.infer-front"' in text, (
        "\nconfigmap-caddy-infer-front.yaml no longer loads the Caddyfile via "
        '.Files.Get "files/Caddyfile.infer-front" — the single-source render '
        "has been removed or renamed."
    )
    reinlined_markers = ("(infer-front-caller-gate)", "handle_path /infer/pull*", "handle_path /infer/api/pull*")
    reinlined = [m for m in reinlined_markers if m in text]
    assert not reinlined, (
        "\nCaddyfile.infer-front listener internals RE-INLINED in "
        f"configmap-caddy-infer-front.yaml: {reinlined}\nEdit the canonical "
