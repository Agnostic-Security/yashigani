...
- **feat(auth): single auth portal with role-based routing** — one login portal routes admins to `/admin` and end users to the chat surface served at the root `/`, with working end-user logout. Deny-by-default is preserved throughout.
