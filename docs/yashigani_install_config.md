...
| `YASHIGANI_DB_AES_KEY` | **Yes** | 64-char hex string | 32-byte AES key (hex-encoded) used by pgcrypto `pgp_sym_encrypt` to encrypt sensitive values at rest (e.g. HIBP API key in `auth_settings` table). Auto-generated by `install.sh`. **Required:** the backoffice service will refuse to start if this variable is missing or empty. Generate with: `openssl rand -hex 32`. |
...
| `gateway.image.tag` | Gateway image tag | `v2.0.0` |
| `backoffice.image.tag` | Backoffice image tag | `v2.0.0` |
