...
**Prompt Injection Worms** — Injected commands escape in LLM responses and propagate to downstream agents, creating a worm that chains through agent-to-agent calls. **Yashigani's defense:** Bidirectional inspection (v2.20+) catches injected commands in responses before they propagate. Response inspection pipeline is fail-closed (v2.20+) — if inspection fails, the response is blocked. Container-per-identity isolation (v2.0+) means even if one agent is compromised, its context cannot reach other agents. Every agent communication audited at ingress and egress (v3.0+ every-hop OPA), proving the worm chain cannot propagate undetected.
