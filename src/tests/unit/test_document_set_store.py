...
def test_set_st_03_set_salt_correlates_file_salt_isolates():
    ...
    ta = TokenAssigner(file_a, secret=None).token_for("v@x.com")
    tb = TokenAssigner(file_b, secret=None).token_for("v@x.com")
    assert ta != tb
    # Set salt: same value → SAME token across files (correlation).
    sa = TokenAssigner(set_salt, secret=None).token_for("v@x.com")
    sb = TokenAssigner(set_salt, secret=None).token_for("v@x.com")
    assert sa == sb
