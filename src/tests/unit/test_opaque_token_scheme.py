...
def _pipeline(**kw) -> DocumentInspectionPipeline:
    # txt/csv re-render runs in the default registry's local worker; no jail
    # needed for these flat formats.
...
def test_output_holder_cannot_reverse_without_original():
    ...
    # deployment secret; even WITH the salt, a wrong candidate value does not
    # reproduce the token, so dictionary attack on an unknown value cannot
    # confirm a hit.  (Salt-only fallback here means secret=None, the weakest
    # case — and it STILL requires the exact value to reproduce the token.)
    salt = r.doc_hash
    tok = next(t for t, v in r.correspondence_table.rows.items() if v == _EMAIL)
    wrong = derive_token(salt, "wrong@guess.com", secret=None)
