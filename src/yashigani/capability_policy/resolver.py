...
Identity boundary — user_id, never email
    The inbound caller identifier (user_id) is the identity_id (idnt_{12hex})
    from the identity registry (set by the gateway boundary resolver as
    request.state.ysg_principal.identity_id — X-Yashigani-Identity-Id rail).
...
def resolve_policy(
    ...
    ----------
    user_id:
        Principal identity_id (idnt_{12hex}) from the identity registry.
        Resolved by the gateway boundary resolver (request.state.ysg_principal).
        None or "" → unauthenticated; group and user tiers are skipped; org
        policy (or baseline) is returned.
    rbac_store:
    ...
    try:
        ...
        # rbac_store is keyed by identity_id (idnt_{12hex}) after the 4.1 UID
