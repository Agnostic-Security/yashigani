...
Redis db/3 key schema:
    ...
    rbac:user:{identity_id}   — Redis SET of group IDs the identity belongs to.
                                ...
                                After the 4.1 UID migration (uid_migrations.py)
...
_KEY_USER  = "rbac:user:{}"    # .format(identity_id) — idnt_{12hex} after 4.1 migration
...
class RBACStore:
    ...
    def add_group(self, group: RBACGroup) -> None:
        ...
        # After the 4.1 UID migration, group.members contains identity_ids
    ...
    def remove_group(self, group_id: str) -> None:
        ...
        # After 4.1 UID migration, group.members contains identity_ids.
        for identity_id in group.members:
    ...
    def get_user_groups(self, identity_id: str) -> list[RBACGroup]:
        ...
        identity_id must be the opaque UID (idnt_{12hex}) from the identity
        registry.  After the 4.1 UID migration the user→group index is
    ...
    def to_opa_document(self) -> dict:
        ...
        After the 4.1 UID migration, group.members contains identity_ids
        (idnt_{12hex}), NOT emails.  The user_groups map is keyed by
        identity_id, matching the OPA rbac.rego input.session.identity_id check.
        """
        groups_doc: dict = {}
