...
def _gen_egress_forwarder_compose(
    ...
    *,
    ...
    lines = [
        ...
        "    # NET_ADMIN) deliberately bypassed; NET_ADMIN stays dropped.",
        '    entrypoint: ["caddy"]',
        ...
        "    cap_drop:",
        "      - ALL",
        "    # NET_BIND_SERVICE mirrors the main caddy service: the upstream caddy",
        "    # binary carries a cap_net_bind_service FILE capability, and under",
        "    # cap_drop:ALL + no-new-privileges the kernel refuses to exec a",
        "    # file-capability'd binary whose caps sit outside the bounding set",
        "    # (`exec /usr/bin/caddy: operation not permitted`). Re-adding the one",
        "    # cap the binary already carries unblocks exec; the forwarder still",
        "    # listens on the unprivileged port and NET_ADMIN remains dropped.",
        "    cap_add:",
        "      - NET_BIND_SERVICE",
        "    read_only: true",
        "    # L3 — IPv6 default-deny",
...
def _gen_compose_override_shape_c(
    ...
    *,
    ...
    lines = [
        ...
        "    # YSG-RISK-138 (Su, 2026-07-27): this target is NESTED under caddy's",
        "    # OWN base `./secrets:/run/secrets:ro` mount (docker/docker-compose.yml)",
        "    # — Docker/Podman must create the mountpoint directory inside that",
        "    # ALREADY-read-only bind at container-create time, which fails hard",
        "    # (\"mkdir ...: read-only file system\", same class as the install.sh",
        "    # YSG-RISK-053 single-file mountpoint-stub finding, runtime-verified",
        "    # Docker 29.4.1) unless the directory already exists on the host",
        "    # UNDERNEATH ./secrets BEFORE this override is applied. The approve",
        "    # transaction (backoffice/mcp_onboard.py _agent_container_deploy_hint)",
        "    # emits a `mkdir -p docker/secrets/svid/<tenant>/<server>` command as",
        "    # the FIRST step of the documented onboarding ceremony for exactly",
        "    # this reason — mirrors the non-crashing pattern (an inert mountpoint",
        "    # pre-created on the host) rather than mounting somewhere caddy has no",
        "    # pre-existing read-only cover (the egress-forwarder services take the",
        "    # latter route — no whole-tree /run/secrets:ro mount on those). `:ro`",
        "    # here: caddy only ever READS the leaf; the sidecar's OWN mount of the",
        "    # SAME named volume (see _gen_svid_sidecar_service) stays writable.",
        "    volumes:",
        "      - %s:%s/%s/%s:ro" % (
            _mcp_svid_volume_name(tenant_id, agent_name), _MCP_SVID_MOUNT_ROOT,
