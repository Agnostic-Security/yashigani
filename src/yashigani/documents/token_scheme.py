...
(``/run/secrets/`` native secrets / KMS), never hardcoded.  When it is absent
the scheme falls back to **salt-only** keying (``HMAC`` keyed on the per-file
salt itself) — still per-file-unique and opaque, but without the cross-deployment
unlinkability the secret adds; the keyed form is strongly preferred and the
absence is surfaced (not silently ignored).
...
def derive_token(
    ...
    *,
    ...
    The HMAC *key* is the deployment ``secret`` when provisioned (keyed form,
    preferred), else the per-file salt itself (``doc_hash`` — salt-only fallback:
    still per-file-unique + opaque, lacks cross-deployment unlinkability).
    ...
    """
    key = secret if secret else doc_hash.encode("ascii")
