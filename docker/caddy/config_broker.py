...
def render_mcp_route(
    ...
    return (
        ...
        "            # FINDING-V412-ONBOARDING-ROBUSTNESS #1 fix (2026-07-21):\n"
        "            # forward_auth's auth-subrequest is INDEPENDENT of the\n"
        "            # request_header directive above — it does NOT inherit\n"
        "            # X-SPIFFE-ID set there (empirically proven against a live\n"
        "            # caddy 2.11.4 binary: only header_up entries declared\n"
        "            # inside forward_auth reach the auth backend). Without this\n"
        "            # line every caller (agent leaf AND gateway mesh identity)\n"
        "            # got 401 no_spiffe_id from /auth/verify-mcp even though the\n"
        "            # TLS handshake + forward_auth hop were both live.\n"
