...
#
# -----------------------------------------------------------------------------
# Modified by Agnostic Security Ltd, 2026-07-18: this file is a first-party
# fork of upstream podman-compose 1.5.0 (containers/podman-compose), forked as
# "podman-compose-ysg" per GPL-2.0 Section 2(a) change-marking requirements.
# This fork is NOT affiliated with, and is not endorsed by, the Podman /
# containers project or Red Hat. It remains licensed entirely under GPL-2.0-
# only (see LICENSE in this directory) — Agnostic Security Ltd's changes do
...
# AS-FIX (2026-07-18): version suffix per GPL-2.0 fork-naming hygiene
# (Petra memo §3.1) — this is NOT bare "podman-compose"; it is
# "podman-compose-ysg" 1.5.0+ysg.1, a first-party-patched fork.
__version__ = "1.5.0+ysg.1"
...
async def container_to_args(
    ...
    for sec_item in sec:
        # AS-FIX-1 (2026-07-18): security_opt was the only path-bearing
        # compose key in this file with no path normalization — env_file
        # (below, ~line 1176) and get_secret_args() both do
        # os.path.realpath(os.path.join(dirname, path)) before use;
        # security_opt passed its value straight through verbatim. A
        # relative seccomp=<path> (the compose-file default form, e.g.
        # "seccomp=./seccomp/yashigani.json") was therefore resolved by
        # podman itself against podman's own cwd rather than the compose
        # project directory, which is not guaranteed to be the same thing
        # (podman-machine client/server split, `podman compose` invoked
        # from an unrelated cwd, etc.) — producing an ENOENT-class failure
        # forcing a fallback to seccomp=unconfined. Fix: mirror the
        # existing env_file/get_secret_args realpath pattern, applied only
        # to the "seccomp=" key (the only key= form that names a
        # filesystem path; "unconfined" and apparmor/label profile names
        # are not paths and must not be realpath()'d).
        if sec_item.startswith("seccomp=") and sec_item != "seccomp=unconfined":
            _seccomp_path = sec_item[len("seccomp="):]
            if _seccomp_path:
                sec_item = "seccomp=" + os.path.realpath(
                    os.path.join(dirname, os.path.expanduser(_seccomp_path))
                )
...
class ServiceDependency:
    # AS-FIX-3 (2026-07-18): `required` added as a stored attribute.
    # Root cause: the compose-spec `depends_on.<svc>.required: false` field
    # survived YAML normalization (normalize_service() only does
    # v.setdefault('condition', 'service_started') and leaves the rest of
    # the per-dependency dict alone) but was DISCARDED here, at graph
    # construction, because __init__ only ever accepted name+condition.
    # flat_deps() below built every ServiceDependency from
    # `v["condition"]` alone, never reading `v.get("required")` — so by the
    # time check_dep_conditions() ran, there was no way to know a missing
    # dependency was intentionally optional. Default True preserves
    # docker-compose-spec semantics (required unless explicitly false).
    def __init__(self, name: str, condition: str, required: bool = True) -> None:
        self._name = name
    ...
    def __hash__(self) -> int:
        ...
        # (identity intentionally excludes `required` — same name+condition
        # dependency reached via two paths, e.g. depends_on + links, should
        # still dedupe as one set member; `required` is metadata about the
        # dependency, not part of what makes two deps distinct)
...
def flat_deps(services: dict[str, Any], with_extends: bool = False) -> None:
    ...
    for name, srv in services.items():
        ...
        # AS-FIX-3 (2026-07-18): thread `required` through instead of
        # discarding it — see ServiceDependency.__init__ comment above.
        # Default True matches compose-spec semantics when `required` is
        # absent from the depends_on entry.
        deps_ls = [
...
# AS-FIX-2 (2026-07-18): real topological sort over `_deps`, replacing two
# call sites that both sorted services/containers by COUNT of direct
# dependencies (`len(deps)`), not real dependency order. A service with
# fewer *direct* deps can have a *transitive* dependency with more direct
# deps of its own, and would sort before it — producing
# `podman create --requires=<not-yet-created>` at container-create time
# (probe Defect 2: localhost_caddy_1 / extractor-svc / backoffice created
# before a dependency they required). Kahn's algorithm, deterministic:
# ties are broken by original services-dict insertion order so behaviour
# stays reproducible across runs of the same compose file.
def _topological_sort_service_names(services: dict[str, Any]) -> list[str]:
    """
    Return service names ordered so every service appears after all of its
    (already-recursively-expanded, via flat_deps/rec_deps) dependencies.
    Falls back to appending any service left over from a dependency cycle
    in original order, rather than silently dropping it — a true cycle is
    a compose-file authoring error this function surfaces, not masks.
    """
    order_index = {name: i for i, name in enumerate(services)}
    in_degree: dict[str, int] = dict.fromkeys(services, 0)
    dependents: dict[str, list[str]] = {name: [] for name in services}

    for name, srv in services.items():
        for dep in srv.get("_deps", ()):  # dep: ServiceDependency
            if dep.name == name or dep.name not in services:
                continue
            in_degree[name] += 1
    ...
    ready = deque(
        sorted((n for n, deg in in_degree.items() if deg == 0), key=lambda n: order_index[n])
    )
    result: list[str] = []
    while ready:
        name = ready.popleft()
    ...
    if len(result) != len(services):
        ordered = set(result)
        remaining = [name for name in services if name not in ordered]
        log.warning(
            "WARNING: dependency cycle detected among services (%s) — "
            "appending in original file order rather than silently dropping",
            ", ".join(remaining),
        )
...
class PodmanCompose:
    ...
    def _parse_compose_file(self) -> None:
        ...
        # AS-FIX-2 (2026-07-18): was `sorted([(len(srv["_deps"]), name) ...])`
        # — sort-by-dependency-COUNT, not real order. See
        # _topological_sort_service_names() for root cause + rationale.
        service_names = _topological_sort_service_names(services)
        ...
        # AS-FIX-2 (2026-07-18): was
        # `given_containers.sort(key=lambda c: len(c.get("_deps", [])))`
        # — the actual site that produced probe Defect 2's
        # `podman create --requires=<not-yet-created>` failures. Order
        # containers by real topological rank of their owning service
        # (each container copies its service's `_deps` via **service_desc,
        # so cnt["service_name"] is the correct key); tie-break by
        # container name for deterministic ordering among replicas of the
        # same service.
        _svc_rank = {name: i for i, name in enumerate(_topological_sort_service_names(services))}
        given_containers.sort(
            key=lambda c: (_svc_rank.get(c.get("service_name", c["name"]), len(_svc_rank)), c["name"])
        )
    ...
    for condition in ServiceDependencyCondition:
        ...
        for d in deps:
            if d.condition == condition:
                ...
                # AS-FIX-3 (2026-07-18): `d.name` may legitimately not be a
                # key in container_names_by_service — e.g. a service
                # excluded by --profile (probe Defect 3: `ollama-init`
                # KeyError when limited to the demo-mcp profile). Before
                # this fix, `required` was discarded at graph-construction
                # time (see ServiceDependency/flat_deps above), so there
                # was no way to distinguish "optional dependency, correctly
                # absent" from "required dependency the ordering fix above
                # failed to create" — a bare except-KeyError guard here
                # would have silently swallowed the latter, a genuine bug,
                # as if it were the former. Now that `required` survives
                # graph construction: honor it explicitly, and fail loudly
                # (not a raw KeyError) for a genuinely-missing required dep
                # rather than mask it.
                if d.name not in compose.container_names_by_service:
                    if not d.required:
                        log.debug(
                            "Skipping wait for optional dependency '%s' "
                            "(required: false, not present — e.g. excluded "
                            "by --profile)",
                            d.name,
                        )
                        continue
                    raise RuntimeError(
                        f"Required dependency '{d.name}' (condition={condition.value}) "
                        "has no containers — it was not created. This means either "
                        "the service is missing from the compose file/selected "
                        "--profile, or (if this recurs) a dependency-ordering "
                        "regression. Add 'required: false' under depends_on if "
                        "this dependency is genuinely optional."
                    )
